...
public class EmployeeCollection extends EmployeeMenu {

	public EmployeeCollection() {
		collection.setText("重整收藏列表");
		loadList();
	...
	public void loadList() {
		try {
			...
			Tester.rs = Tester.statement.executeQuery(
					"SELECT Work.id, Work.title, department, name FROM Work, Company, Collection WHERE Work.id = Collection.work AND Work.company = Company.id AND Collection.user = "
							+ Tester.ID);
	...
	public void setWork(int id) {
		try {
			Tester.connectSQL();
			Tester.rs = Tester.statement.executeQuery(
					"SELECT Work.title, Company.name, Work.department, Work.location, Work.type, Work.worktime, Work.wage, Work.experience, Work.major, Work.language, Work.skills, Work.other, Work.description, Work.responsibilities, Work.contact FROM Work, Collection, Company WHERE Collection.work = Work.id AND Work.company = Company.id AND Collection.user ="
							+ Tester.ID + " AND Collection.work = " + id);
			String content = null;
			while (Tester.rs.next()) {
			...
			while (Tester.rs.next()) {
				content += "<p style='font-family: sans-serif; padding: 30'>"
						...
						+ "</span><br><br><br>" + "<span style='font-size:17'><b>總覽:</b><br><br>"
	...
	class searchListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				...
				Tester.connectSQL();
				Tester.rs = Tester.statement.executeQuery(
						"SELECT Work.*, Company.name FROM Work, Company, Collection WHERE Company.name LIKE '%"
								+ search.getText() + "%' AND Work.company = Company.id AND Collection.user = "
								+ Tester.ID + " AND Collection.work = Work.id OR Work.title LIKE '%" + search.getText()
								+ "%' AND Work.company = Company.id AND Collection.user = " + Tester.ID
								+ " AND Collection.work = Work.id OR Work.department LIKE '%" + search.getText()
								+ "%' AND Work.company = Company.id AND Collection.user = " + Tester.ID
								+ " AND Collection.work = Work.id;");
				while (Tester.rs.next()) {
					l.addElement("<html><br><font size= '6'; font color= '#2C64BC'><b>" + Tester.rs.getString("title")
					+ "</b></font><br><font size= '4'; font color= '#666666'>" + Tester.rs.getString("name")
					+ "	" + Tester.rs.getString("department") + "<br><br></font></html>");
